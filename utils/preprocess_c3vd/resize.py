...
def resize_image(path, target_width, target_height):
    ...
    for depth_file_path in depth_file_paths:
        full_path = os.path.join(depth_folder_path, depth_file_path)
        depth = cv2.imread(full_path, cv2.IMREAD_UNCHANGED)
        depth = cv2.resize(depth, (target_width, target_height), interpolation=cv2.INTER_NEAREST)
        cv2.imwrite(full_path, depth)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target_width", default=640, type=int, help="Target width for the images.")
    parser.add_argument("--target_height", default=512, type=int, help="Target height for the images.")
